...
export default function handler(req, res) {
  ...
  axios
    .get(`https://idev.games/most-liked-ad-free-games/page${page}`)
    .then((response) => {
      ...
      $(".paginationBox", html).each(function () {
        ...
        const totalPages = $(this).find('.pagiStat').text().match(/of (\d+)/)[1];
      ...
      $(".gameImageFeed", html).each(function () {
        ...
        const description = $(this)
          ...
          .text()
          .trim();
        const thumbnail = URL_BASE + $(this).find("div > div > a > img").attr("src");
        const backgroundStyle = $(this).css('background');

        // Extract the URL from the background style
        const cover = URL_BASE + backgroundStyle.match(/url\(['"]?([^'"]+)['"]?\)/)[1];
